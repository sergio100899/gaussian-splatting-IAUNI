...
class GaussianModel:
    ...
    def get_smallest_axis(self, return_idx=False):
        ...
        smallest_axis_idx = self.get_scaling.min(dim=-1)[1][..., None, None].expand(
            -1, 3, -1
        )
    ...
    def render_depth_and_normal(self, viewpoint_camera, scaling_modifier=1.0):
        """
        ...
        raster_settings_depth = GaussianRasterizationSettings(
            ...
            prefiltered=False,
            debug=False,
            antialiasing=False,  # Añadir este argumento
        )
        ...
        depth_features = points_in_camera_space[..., 2:3].repeat(
            1, 3
        )  # Usar Z como feature

        # Renderizar profundidad
        (
            rendered_depth_image,
            _,
            _,
        ) = rasterizer_depth(
            means3D=means3D,
    ...
    def densify_and_split(
        ...
        )  # points that are bigger than threshold

        stds = self.get_scaling[selected_pts_mask].repeat(N, 1)  # new gaussians scales
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        selected_pts_mask = torch.logical_and(
            ...
            torch.max(self.get_scaling, dim=1).values
            <= self.percent_dense * scene_extent,
        )  # points that are smaller than threshold
